...
opts_knit$set(root.dir = normalizePath("/home/fmarotta/MAT0043/results/"))
```
...
g1 <- ggplot(expr_bid) +
  ...
  geom_vline(xintercept = -sd(expr_bid$EXPRESSION),
             ...
             lwd = .7) +
  geom_density() +
  labs(subtitle = paste("Histogram & Kernel Density Estimate"),
       x = "Expression",
       y = "Density")#  +
...
g2 <- ggplot(expr_bid) +
  ...
  geom_qq(color = "black") +
  labs(subtitle = paste("Normal Q-Q plot"),
       x = "Theoretical",
       y = "Expression")#  +
  # theme(plot.title = element_text(hjust = .5))
```
...
```{r}
grid.arrange(g1, g2, ncol = 2, top = "Expression of BID")
```
...
pc.summ <- cbind(PC = 1:nrow(tba_esum), as.data.table(t(summary(pc)$importance)))
```
...
g1 <- ggplot(pc.summ) +
  ...
  xlim(c(0, 30)) +
  labs(subtitle = paste("Screeplot of the TBA (first 30 PCs)")) #+
  # theme(plot.title = element_text(hjust = .5))
...
```{r tba_biplot}
g2 <- ggbiplot(pc, var.axes = F, labels = tba_esum$IID, labels.size = 2) +
  coord_equal(ratio = .5) +
  labs(subtitle = paste("Biplot of the TBA"))# +
  # theme(plot.title = element_text(hjust = .5))
```

```{r}
grid.arrange(g1, g2, ncol = 2, top = "PCA of the affinities for BID")
```
...
tigar <- fread("../../bacmod/models/tigar/tba/gtex/DPR_CHR22/CHR22_DPR_training_info.txt")
...
```{r nested_cv}
m <- readRDS("models/ranger/tba/gtex/chr22/GM12878.Cells_EBV_transformed_lymphocytes.perf_inner.Rds")

len <- sort(unique(unlist(apply(a, 1, unique))), decreasing = T)[3]
lengths <- sapply(m, function(gene) sapply(gene, function(fold) nrow(fold$aggr)))
deltas <- sapply(m, function(gene) sapply(gene, function(fold) max(fold$aggr$pred_perf_R2, na.rm = T) - min(fold$aggr$pred_perf_R2, na.rm = T)))

long_arr_ind <- which(lengths == len, arr.ind = T)
long <- which(lengths == len)
ind <- long_arr_ind[order(deltas[long], decreasing = T)[1], ]


gene <- ind[2]
fold <- ind[1]
i <- m[[gene]][[fold]]$aggr
j <- i[, .(pred_perf_R2 = mean(pred_perf_R2, na.rm = T)), by = c("mtry", "nodesize")]
ggplot() +
  # aes(x = mtry, y = nodesize, z = pred_perf_R2, fill = pred_perf_R2) +
  geom_tile(data = i, aes(x = mtry, y = nodesize, fill = pred_perf_R2), width = 11, height = 2.3) +
  # geom_contour(data = j, aes(x = mtry, y = nodesize, z = pred_perf_R2), bins = 50) # +
  geom_text(data = i[1, ], aes(x = mtry, y = nodesize), label = "Default", vjust = -1.2) +
  geom_text(data = i[which.max(pred_perf_R2)], aes(x = mtry, y = nodesize), label = "Best", hjust = -.5) +
  scale_fill_gradient(low = "red", high = "blue") +
  labs(title = expression("Explored RF Parameters and Corresponding "*rho^2),
       x = "mtry",
       y = "nodesize",
       fill = expression("CV "*rho^2)) +
  theme(plot.title = element_text(hjust = .5))
```
